...
try {
    ...
    $sql_curtida = "DELETE FROM curtidas WHERE id_perfil_usuario = ?";
    $stmt_curtida = $conn->prepare($sql_curtida);
    $stmt_curtida->bind_param("s", $id_usuario);
    if(!$stmt_curtida->execute()) {
        throw new Exception("Falha ao excluir curtidas: " . $stmt_curtida->error);
    }
    $stmt_curtida->close();

    $sql_comentario = "DELETE FROM comentario WHERE id_usuario = ?";
    $stmt_comentario = $conn->prepare($sql_comentario);
    $stmt_comentario->bind_param("s", $id_usuario);
    if(!$stmt_comentario->execute()) {
        throw new Exception("Falha ao excluir comentario: " . $stmt_comentario->error);
    }
